#!/usr/bin/env python3
"""
Monitor de Voos: Curitiba (CWB) -> Bogotá (BOG)
Ida: 28/10/2026 | Volta: 09/12/2026 | 2 adultos
"""
...
LOG_FILE      = os.path.join(os.path.dirname(__file__), "data", "historico_bog.json")
TIMEOUT_KAYAK = 30
NUM_ADULTOS   = 2

# Busca 1: Menor preço geral (1 ou 2 paradas)
URL_MENOR_PRECO = (
    "https://www.kayak.com.br/flights/CWB-BOG/2026-10-28/2026-12-09/"
    "2adults?sort=price_a&fs=stops=1,2"
)

# Busca 2: Melhor rota (exatamente 1 parada em cada perna)
URL_MELHOR_ROTA = (
    "https://www.kayak.com.br/flights/CWB-BOG/2026-10-28/2026-12-09/"
...
def buscar(url, label):
    ...
    try:
        ...
        if cards:
            card = cards[0]
            dados["detalhes_voo"] = extrair_detalhes_card(card)
            preco_casal = extrair_preco_card(card.text)
            if preco_casal:
                ...
                print(f"⚠️  [{label}] Preço não encontrado.")
        else:
...
def salvar_historico(r):
    ...
    print(f"📝 Histórico BOG salvo com {len(hist)} registros.")


if __name__ == "__main__":
    print("=" * 50)
    print("✈️  MONITOR CWB → BOG | 28/10 - 09/12/2026")
    print("=" * 50)

    menor  = buscar(URL_MENOR_PRECO, "Menor Preço (1-2 paradas)")
    melhor = buscar(URL_MELHOR_ROTA, "Melhor Rota (1 parada)")

    registro = {
        "timestamp":    datetime.datetime.now(datetime.timezone.utc).isoformat(),
        "menor_preco":  menor,
        "melhor_rota":  melhor,
        "status":       "ok" if menor["status"] == "ok" or melhor["status"] == "ok" else "erro",
    }
